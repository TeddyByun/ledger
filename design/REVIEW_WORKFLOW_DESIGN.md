...
## 0. 구현 현황 (as-built, 2026-08-01)

**본 설계의 "잡 단위 검토 화면 + 확정 API"는 구현하지 않았다.** 같은 목적을 목록 화면에서 달성한다.

| 설계 | 현행 구현 |
|------|-----------|
| `GET /imports/{jobId}/pending` 로 잡별 검토 대기 조회 | 엔드포인트는 존재하나 화면 미사용 |
| 검토 화면에서 선택 → **확정**(`PATCH /imports/{jobId}/classify`) | **미구현**. 대신 은행/카드 거래 화면에서 인라인 분류(`PATCH /bank-transactions/:id`)·**일괄 분류**(`POST …/bulk-classify`)·**일괄 삭제**(`POST …/bulk-delete`) |
| 확정 시 규칙 학습 체크박스 → `merchant_category_map` 추가 | **명시적 화면**으로 분리 — 관리>**자동분류 키워드**(`/classify-keywords` CRUD) |
| 추천 분류(✨) 미리 채움 | 업로드 시점에 이미 자동분류가 끝나며, 이후 **[자동분류] 버튼**으로 재실행(정기지출→이력→키워드 순, ARCHITECTURE §5.1) |
| 제외 처리(이체/카드대금)를 사용자가 선택 | **자동 대사**로 처리 — '분류 제외'(18/19) 분류 매핑(DATABASE §7.1.1) |
| — (설계에 없음) | **분류 불일치**(`GET …/category-conflicts`) — 같은 내용이 서로 다른 분류로 잡힌 건을 모아 교정 |

- 확정(commit) 개념이 없다: 분류하면 즉시 `transaction` 생성·연결 + 영향 월 `rebuild`.
- 되돌리기(undo)·잡 단위 롤백도 미구현이며, 대신 일괄 삭제로 처리한다.
- 아래 §1~ 는 **향후 잡 단위 검토를 도입할 때의 설계안**으로 남긴다.
