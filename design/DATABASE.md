...
#### 지출 분류 코드 시드 데이터
> 실제 명세서(하나/현대/삼성 3월) 소비 패턴을 반영해 세분화. `packages/shared/category-codes.ts` 가 단일 소스.

| code | parent_code | name | depth |
|------|-------------|------|-------|
| 01 | (NULL) | 대출 | 1 |
| 0101 · 0102 | 01 | 이자 · 원금 | 2 |
| 02 | (NULL) | 투자·저축 | 1 |
| 0201 · 0202 | 02 | 투자 · 적금·청약 | 2 |
| 03 | (NULL) | 보험 | 1 |
| 04 | (NULL) | 공과금·주거 | 1 |
| 0401 · 0402 | 04 | 관리비 · 전기·가스·수도 | 2 |
| 05 | (NULL) | 생활 | 1 |
| 0501 | 05 | **외식** | 2 |
| 0502 | 05 | 카페·간식 | 2 |
| 0503 | 05 | 식료품·마트 | 2 |
...
> **대분류 통폐합** — 지출 대분류는 01~08 · 10 · 11 (09 차량·12 여가 폐지).
> - 여가(12): 구독·디지털 → `생활(05)` 하위 `0507`, 취미·문화 계열 → `생활(05)` 하위 `0505 여가·취미·문화`.
> - 차량(09): 주유 → `교통(08)` 하위 `0804`, 주차·정비 → `교통(08)` 하위 `0805`.
...
-- 지출 분류 코드 시드 데이터 (단일 소스: packages/shared/category-codes.ts)
INSERT INTO category (code, parent_code, name, type, depth, sort_order) VALUES
  ('01', NULL, '대출',     'expense', 1,  1),
  ('0101','01','이자',     'expense', 2,  1), ('0102','01','원금','expense',2,2),
  ...
  ('0503','05','식료품·마트','expense',2,  3),
  ('0504','05','생활용품', 'expense', 2,  4),
  ('0505','05','ATM 출금', 'expense', 2,  5),
  ('0506','05','기타',     'expense', 2,  6),
  ...
  ('0901','09','주유',     'expense', 2,  1), ('0902','09','주차·정비','expense',2,2),
  ...
  ('1201','12','구독·디지털','expense',2,  1), ('1202','12','취미·문화','expense',2,2);
