...
> **분류 제외(집계 제외)**: 지출 `18`·수입 `19` = `분류 제외` 대분류. 이 분류(및 하위)로 지정된 거래는 전체 거래·합계·대시보드 추이 집계에서 제외된다(`common/exclude-category.ts`의 `EXCLUDE_CATEGORY_NAMES` 로 이름 매칭, 구버전 `집계제외` 호환).
